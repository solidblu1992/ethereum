...
contract RingCTToken is MLSAG_Verify {
	...
	mapping (uint256 => uint256) public dhe_points;
	uint256 public dhe_point_count;
        ...
        public returns (bool success)
    {
        ...
        if (!VerifyMLSAG(HashSendMsg(dest_pub_keys, values,dest_dhe_points), I, P, signature)) return false;
		...
		for (v.i = 0; v.i < (dest_pub_keys.length / 2); v.i++) {
			...
			dhe_points[dhe_point_count] = v.point3[0];			//Store DHE point (for calculating stealth address)
			dhe_point_count++;
		}
    ...
    //Utility Functions
    function HashSendMsg(uint256[] dest_pub_keys, uint256[] output_commitments, uint256[] dest_dhe_points)
        public pure returns (bytes32 msgHash)
    {
        msgHash = keccak256(Keccak256OfArray(dest_pub_keys), Keccak256OfArray(output_commitments), Keccak256OfArray(dest_dhe_points));
    }
