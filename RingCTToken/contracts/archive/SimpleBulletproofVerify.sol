...
import "./libSimpleBulletproofStruct.sol";

contract SimpleBulletproofVerify is ECMathInterface {
	...
	function VerifySimpleBulletproof(SimpleBulletproofStruct.Data bp)
		...
		for (i = 0; i < v.N; i++) {
            ...
            v.gs = sAdd(v.gs, v.z);
            v.hs = sSub(v.hs, sMul( sAdd(sMul(v.z, v.vpy[i]), sMul(sSq(v.z), v.vp2[i]) ), v.vpyi[i]));
            
            v.IPG = ecMath.AddMultiply(v.IPG, [v.Gi[index], v.Gi[index+1]], v.gs);
            v.IPH = ecMath.AddMultiply(v.IPH, [v.Hi[index], v.Hi[index+1]], v.hs);
            index += 2;
		}
		
		//Verify Pprime equality
		v.Left = ecMath.AddMultiply(bp.A, bp.S, v.x);
		v.Left = ecMath.AddMultiplyG1(v.Left, sNeg(bp.mu));
		
		index = 0;
		for (i = 0; i < v.logN; i++) {
		    v.Left = ecMath.AddMultiply(v.Left, [bp.L[index], bp.L[index+1]], sSq(v.w[i]));
		    v.Left = ecMath.AddMultiply(v.Left, [bp.R[index], bp.R[index+1]], sSq(sInv(v.w[i])));
		    index += 2;
		}
		
		v.Left = ecMath.AddMultiplyH(v.Left, sMul(bp.t, v.x_ip));
		
		v.Right = ecMath.Add(v.IPG, v.IPH);
		v.Right = ecMath.AddMultiplyH(v.Right, sMul(sMul(bp.a, bp.b), v.x_ip));

		if (!ecMath.Equals(v.Left, v.Right)) {
		    return false;
		}
		else {
		    return true;
	...
	function VerifySimpleBulletproof(uint256[] argsSerialized) public returns (bool) {
		return VerifySimpleBulletproof(SimpleBulletproofStruct.Deserialize(argsSerialized));
	}
