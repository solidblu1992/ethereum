...
class RingCTToken:
    ...
    def GenerateNewCommitment(self, value, blinding_factor):
        point = multiply(G1, blinding_factor)
        temp = multiply(H, value)
        point = add(point, temp)
        self.MyCommitments = self.MyCommitments + [point]

    def GenerateNewCommitments(self, values, blinding_factors):
        assert(len(values) == len(blinding_factors))
        for i in range(0, len(values)):
            self.GenerateNewCommitment(values[i], blinding_factors[i])
...
def RingCTTest():
    ...
    rct.GenerateNewCommitments([10**18]*5, [getRandom()]*5)
    rct.PrintAddresses()
